//
//  Created by Joojin Abaoag on 4/22/21. Based on HoRNDIS
//
...
#endif
#define LOG(verbosity, s, ...) do { if (verbosity >= DEBUGLEVEL) IOLog("netlink: %s: " s "\n", __func__, ##__VA_ARGS__); } while(0)


OSDefineMetaClassAndStructors(netlink, IOUserNetworkEthernet);
...
bool netlink::init(OSDictionary *properties) {
    extern kmod_info_t kmod_info;  // Getting the version from generated file.
    LOG(V_NOTE, "netlink tethering driver for Mac OS X, %s", kmod_info.version);
...
bool netlink::start(IOService *provider) {
    LOG(V_DEBUG, ">");
...
bool netlink::willTerminate(IOService *provider, IOOptionBits options) {
    LOG(V_DEBUG, ">");
...
void netlink::stop(IOService *provider) {
    LOG(V_DEBUG, ">");
...
bool netlink::openUSBInterfaces(IOService *provider) {
...
void netlink::closeUSBInterfaces() {
...
IOService *netlink::probe(IOService *provider, SInt32 *score) {
...
IOService *netlink::probeDevice(IOUSBHostDevice *device, SInt32 *score) {
...
bool netlinkInterface::init(IONetworkController *controller, int mtu) {
    maxmtu = mtu;
    if (IOUserNetworkEthernet::init(controller) == false) {
        return false;
    }
...
bool netlinkInterface::setMaxTransferUnit(UInt32 mtu) {
    if (mtu > maxmtu) {
    ...
    IOUserNetworkEthernet::setMaxTransferUnit(mtu);
    return true;
}

/* Overrides IOEthernetController::createInterface */
IONetworkInterface *netlink::createInterface() {
    LOG(V_DEBUG, ">");
    netlinkInterface *netif = new netlinkInterface;
...
bool netlink::createNetworkInterface() {
...
netlink::ReentryLocker::ReentryLocker(IOCommandGate *inGate, bool &inGuard)
...
netlink::ReentryLocker::~ReentryLocker() {
    if (!isInterrupted()) {
        entryGuard = false;
...
IOReturn netlink::enable(IONetworkInterface *netif) {
...
void netlink::disableNetworkQueue() {
...
IOReturn netlink::disable(IONetworkInterface *netif) {
    LOG(V_DEBUG, "begin for thread_id=%lld", thread_tid(current_thread()));
...
bool netlink::createMediumTables(const IONetworkMedium **primary) {
...
bool netlink::allocateResources() {
...
IOOutputQueue *netlink::createOutputQueue() {
    LOG(V_DEBUG, ">");
...
bool netlink::configureInterface(IONetworkInterface *netif) {
    LOG(V_DEBUG, ">");
    IONetworkData *nd;
...
IOReturn netlink::getPacketFilters(const OSSymbol *group, UInt32 *filters) const {
    IOReturn    rtn = kIOReturnSuccess;
...
IOReturn netlink::getMaxPacketSize(UInt32 *maxSize) const {
...
IOReturn netlink::selectMedium(const IONetworkMedium *medium) {
    LOG(V_DEBUG, ">");
...
IOReturn netlink::getHardwareAddress(IOEthernetAddress *ea) {
    LOG(V_DEBUG, ">");
    UInt32      i;
    void *buf;
    unsigned char *bp;
...
IOReturn netlink::setMulticastMode(bool active) {
...
IOReturn netlink::setMulticastList(IOEthernetAddress *addrs,
                                UInt32             count) {
...
IOReturn netlink::setPromiscuousMode(bool active) {
...
UInt32 netlink::outputPacket(mbuf_t packet, void *param) {
...
void netlink::dataWriteComplete(void *obj, void *param, IOReturn rc, UInt32 transferred) {
    netlink    *me = (netlink *)obj;
...
void netlink::dataReadComplete(void *obj, void *param, IOReturn rc, UInt32 transferred) {
    netlink    *me = (netlink *)obj;
 ...
 */
void netlink::receivePacket(void *packet, UInt32 size) {
    mbuf_t m;
    UInt32 submit;
    IOReturn rv;
...
IOReturn netlink::rndisCommand(struct rndis_msg_hdr *buf, int buflen) {
...
int netlink::rndisQuery(void *buf, uint32_t oid, uint32_t in_len, void **reply, int *reply_len) {
    int rc;
...
bool netlink::rndisSetPacketFilter(uint32_t filter) {
    union {
        struct rndis_msg_hdr *hdr;
